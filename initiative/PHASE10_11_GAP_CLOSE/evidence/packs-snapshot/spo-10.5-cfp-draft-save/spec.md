# Spec 10.5 — Public CFP draft save and resume

> **Source Concept**: Implement draft submission save/resume on public CFP without full submit. | **Complexity**: high | **Risk**: high | **Domain**: fullstack
...
- **Status**: Cumulative on dependencies ['10.3', '10.4']
...
| Layer | Current State |
| --- | --- |
| Submissions | decisions/publicCfp modules, D1 submissions status |
| Public UI | PublicCfp.tsx |

## 2. Items In Scope

### Draft
- API command save draft (title-only allowed)
- Status draft; resume loads snapshot
- UI control + confirmation
- Not available when CFP closed
...
| Metric | Criteria |
| --- | --- |
| Save draft | Minimal title persists |
| Resume | Reload restores fields |
| Closed | Draft save disabled when closed |

### Tests that prove these (E9)

- Unit draft command
- e2e draft save/resume
...
|---|---|---|---|---|
| draftTitle | public CFP form | submission draft row | empty invalid | e2e resume |
| snapshot | form fields | form_versions-compatible JSON | n/a | roundtrip unit |

## 10. Traceability

| Requirement | Section | Spec AC | Test |
| --- | --- | --- | --- |
| Constitution souls mapped in phase index | 10.5 | Success metrics | Named tests above |
...
See Named ACs in this spec (or livability matrix for spo-10.5-cfp-draft-save). Each AC maps to a named playwright/unit test file in plan Files-to-Create/Modify. Completion gate requires each AC ticked in plan.
