# Spec 11.0 — Lumen 2 tokens + shared primitives

> **Source Concept**: Extend lumen.css + components/ui/* with Lumen 2 primitives and state anatomy. | **Complexity**: high | **Risk**: high | **Domain**: frontend
...
- **Status**: Cumulative on dependencies ['10.7', '1.4']
...
| Layer | Current State |
| --- | --- |
| Tokens | apps/web/src/styles/lumen.css |
| Shell CSS | apps/web/src/styles/shell.css |
| Pack | design-pack at speakerops/initiative/PHASE10_11_GAP_CLOSE/evidence/design-pack-snapshot/ (checksum: evidence/design-pack-snapshot.SHA256). Do NOT use laptop ChatGPT design-pack paths on the box. tokens.css components.css icons.svg |

## 2. Items In Scope

### Foundation
- Merge semantic type/space/elevation/border/motion into lumen.css (aliases to ratified --lumen-*)
- Icon.tsx first-party 24px paths from icons.svg
- Button, Field, Badge, Card, Alert, Modal, DataTable, EmptyState, PageHeader, Skeleton
- Component state sheet page or test capture
- Token lint: no undefined --lumen vars; no raw hex in pages
...
- Parallel --l2 theme system that replaces --lumen SoT
...
| Prior | 1.4 | Prior section complete |

## 5. Success Metrics

| Metric | Criteria |
| --- | --- |
| Primitives | Listed components exist with state classes |
| Deps | pnpm audit:deps / lockfile no new UI kits |
| Tokens | No undefined CSS vars in shell |

### Tests that prove these (E9)

- lumen.test.ts extended
- component unit tests
- visual state sheet capture script optional
...
| --- | --- | --- | --- |
| Constitution souls mapped in phase index | 11.0 | Success metrics | Named tests above |
...
## Named ACs (S-L2-SYSTEM)

| ID | Assertion |
| --- | --- |
| AC-11.0-A | Button, Field, Badge, Card, Alert, Modal, DataTable, EmptyState, PageHeader, Icon, Skeleton exist under apps/web/src/components/ui/ with state classes |
| AC-11.0-B | lumen.css holds extended tokens; no parallel --l2 theme replacing --lumen SoT |
| AC-11.0-C | State sheet reviewable: route or playwright capture with data-testid=l2-state-sheet |
| AC-11.0-D | Inventory @inv:L2-01 registered; playwright/e2e/lumen2_state_sheet.spec.ts exists and passes F |
| AC-11.0-E | package.json/pnpm-lock no new UI kit/icon/chart/font/DnD runtime deps vs baseline |

## Tests
- unit: lumen token undefined refs
- playwright/e2e/lumen2_state_sheet.spec.ts @inv:L2-01
- lockfile diff gate

## Exact AC→test mapping
See Named ACs in this spec (or livability matrix for spo-11.0-lumen2-foundation-primitives). Each AC maps to a named playwright/unit test file in plan Files-to-Create/Modify. Completion gate requires each AC ticked in plan.
