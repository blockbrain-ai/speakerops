# Spec 11.1 — Admin shell + Overview readiness composition

> **Source Concept**: Recompose AdminShell + Readiness per page-atlas /admin. | **Complexity**: high | **Risk**: high | **Domain**: frontend
...
Recompose AdminShell + Readiness per page-atlas /admin.

## Current Build Contract

- **Status**: Cumulative on dependencies ['11.0']
...
| Layer | Current State |
| --- | --- |
| Shell | apps/web/src/layout/AdminShell.tsx |
| Overview | apps/web/src/pages/Readiness.tsx |

## 2. Items In Scope

### Shell
- Icons from 11.0; account/help area; tablet drawer / mobile nav strategy
### Overview
- Four metrics + readiness + ranked attention queue + quick actions
- Session-expired must not show as alert inside usable shell (redirect/recovery)
...
| Metric | Criteria |
| --- | --- |
| 5-second test | Primary risk + next action visible |
| Mobile | 390px nav usable |
| Auth expiry | Focused recovery not shell+alert |
...
| --- | --- | --- | --- |
| Constitution souls mapped in phase index | 11.1 | Success metrics | Named tests above |
...
See Named ACs in this spec (or livability matrix for spo-11.1-lumen2-shell-overview). Each AC maps to a named playwright/unit test file in plan Files-to-Create/Modify. Completion gate requires each AC ticked in plan.
