# Spec 11.2 — Communications campaign workflow + scale

> **Source Concept**: Recompose Comms.tsx to four-step campaign flow operable at 150 speakers. | **Complexity**: high | **Risk**: high | **Domain**: frontend
...
- **Status**: Cumulative on dependencies ['11.1', '5.3']
...
| Layer | Current State |
| --- | --- |
| Comms | apps/web/src/pages/Comms.tsx, comms-utils.ts |
| API | apps/api/src/modules/comms/** |

## 2. Items In Scope

### Campaign UI
- Steps: Audience, Message, Review, Send
- Audience: search, filters, segments, pagination/virtualization (≤25 visible or virtual)
- Summary count always visible
- Trust-before-send preserved
- No behavior change to scopes/outbox
...
| --- | --- | --- |
| Prior | 11.1 | Prior section complete |
| Prior | 5.3 | Prior section complete |

## 5. Success Metrics

| Metric | Criteria |
| --- | --- |
| Scale | 150 recipients operable without 150 raw checkboxes primary |
| Steps | Four steps present |
| Send | Existing send still works e2e |

### Tests that prove these (E9)

- e2e comms campaign scale
...
| --- | --- | --- | --- |
| Constitution souls mapped in phase index | 11.2 | Success metrics | Named tests above |
...
## Named ACs — J01–J10 regression + scale

| ID | Assertion |
| --- | --- |
| AC-11.2-Jxx | Inventory J01–J10 remain PASS (`comms_keystone.spec.ts` + lumen2 tests) |
| AC-11.2-SCALE | 150 recipients operable without raw 150-checkbox wall; page≤25 or virtualised |
| AC-11.2-SEL | Selection stable across filter/page; review invalidates on audience edit |
| AC-11.2-SEND | Preview required; idempotent send; double-submit guarded |
| AC-11.2-AUTHZ | Non-admin cannot send; event scoped |
| AC-11.2-UI | Audience→Message→Review→Send using 11.0 primitives |

## Exact AC→test mapping
See Named ACs in this spec (or livability matrix for spo-11.2-lumen2-comms-campaign). Each AC maps to a named playwright/unit test file in plan Files-to-Create/Modify. Completion gate requires each AC ticked in plan.

## Field-flow (I16) — campaign

| Field | Source | Consumers | Fallback | Same-value proof |
| --- | --- | --- | --- | --- |
| audienceRules | Audience step UI | segment API → preview → send | empty = block send | J02/J03 count match |
| recipientCount | segment API | Review step display | 0 blocks send | AC-11.2-SCALE |
| subject/body | Message step | preview render + send payload | required | J01 merge fields |
| previewToken | preview API | send gate | missing blocks send | J08 |
| idempotencyKey | client/send | outbox | server generate | J04 double send |
