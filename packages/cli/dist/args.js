...
export function hasJson(args, argv) {
    return args.flags.has("json") || argv.includes("--json") || argv.includes("-j");
}
export function requireOption(args, name, aliases = []) {
    if (args.options.has(name))
        return args.options.get(name);
    for (const a of aliases) {
        if (args.options.has(a))
            return args.options.get(a);
    }
    return undefined;
}
/** Split comma-separated scopes list. */
export function parseScopesList(raw) {
    return raw
        .split(",")
        .map((s) => s.trim())
        .filter((s) => s.length > 0);
