 ...
 */
import { cmdCommsDraft, cmdCommsSend, cmdCommsTemplates, cmdDesignGet, cmdDesignPublish, cmdDesignSet, cmdEventsList, cmdEvalExport, cmdEvalRollup, cmdFilesUpload, cmdFormsCreate, cmdFormsDraft, cmdFormsGet, cmdFormsList, cmdFormsPublish, cmdKeysCreate, cmdMembersInvite, cmdMembersList, cmdMembersSetRole, cmdOpenApi, cmdReportsReadiness, cmdScheduleList, cmdSchedulePlace, cmdScheduleUnschedule, cmdSpeakersUpdateProfile, cmdSubmissionsAssign, cmdSubmissionsBulkDecision, cmdSubmissionsDecision, cmdSubmissionsGet, cmdSubmissionsList, parseArgs, resolveClient, setClientFactoryForTests, } from "./commands.js";
import { EXIT_OK, EXIT_VALIDATION, } from "./exit-codes.js";
import { defaultIo, emitError } from "./output.js";
...
export async function main(argv = process.argv.slice(2), options = {}) {
    ...
    try {
        ...
        const restForParse = resource === "openapi" || resource === "help" || resource === "version"
            ? argv.slice(1)
            : rest;
        const args = parseArgs(resource && verb && resource !== "openapi"
            ? rest
            : resource === "openapi"
                ? restForParse
                : argv);
        ...
        const client = resolveClient(parseArgs(argv));
        const ctx = {
            argv,
            args: parseArgs(resource === "openapi"
                ? argv.slice(1)
                : verb
                    ? argv.slice(2)
                    : argv.slice(1)),
...
async function dispatch(resource, verb, ctx) {
    switch (resource) {
        case "events":
            if (verb === "list")
                return cmdEventsList(ctx);
            break;
        case "reports":
            if (verb === "readiness")
                return cmdReportsReadiness(ctx);
            break;
        case "design":
            if (verb === "get")
                return cmdDesignGet(ctx);
            if (verb === "set")
                return cmdDesignSet(ctx);
            if (verb === "publish")
                return cmdDesignPublish(ctx);
            break;
        case "schedule":
            if (verb === "place")
                return cmdSchedulePlace(ctx);
            if (verb === "list")
                return cmdScheduleList(ctx);
            if (verb === "unschedule")
                return cmdScheduleUnschedule(ctx);
            break;
        case "files":
            if (verb === "upload")
                return cmdFilesUpload(ctx);
            break;
        case "speakers":
            if (verb === "update-profile" ||
                verb === "update" ||
                verb === "profile") {
        ...
        case "comms":
            ...
            if (verb === "draft" || verb === "preview")
                return cmdCommsDraft(ctx);
            if (verb === "send")
                return cmdCommsSend(ctx);
            break;
        case "members":
            if (verb === "list")
                return cmdMembersList(ctx);
        ...
        case "keys":
            if (verb === "create")
                return cmdKeysCreate(ctx);
            break;
        case "forms":
            if (verb === "list")
                return cmdFormsList(ctx);
            if (verb === "get")
                return cmdFormsGet(ctx);
            if (verb === "create")
                return cmdFormsCreate(ctx);
            if (verb === "draft")
                return cmdFormsDraft(ctx);
            if (verb === "publish")
                return cmdFormsPublish(ctx);
            break;
        case "submissions":
            if (verb === "list")
                return cmdSubmissionsList(ctx);
            if (verb === "get")
                return cmdSubmissionsGet(ctx);
            if (verb === "assign")
                return cmdSubmissionsAssign(ctx);
            if (verb === "decision")
                return cmdSubmissionsDecision(ctx);
        ...
        case "eval":
            if (verb === "rollup")
                return cmdEvalRollup(ctx);
            if (verb === "export")
                return cmdEvalExport(ctx);
            break;
        case "openapi":
    ...
    return emitError(ctx.io, `Unknown command: ${resource}${verb ? ` ${verb}` : ""}. See --help or docs/CLI.md`, "VALIDATION_ERROR", EXIT_VALIDATION, ctx.json);
}
