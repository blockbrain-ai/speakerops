...
| `Cfp.FileUpload` | public | eventSlug, formVersionId (required pin — must be the ACTIVE public version, i.e. the exact one Form.GetPublic serves for the slug), fieldKey (required; must be a file-typed field on that version), filename, mime, size, contentBase64 | { fileId, mime, size, filename } — supporting file for public CFP; mime/size allowlist enforced; 400 when the pin is not the active published version or the named field is not a file field — no anonymous D1 blob writes via stale versions or non-file forms. The binding (form_version_id + field_key, migration 0033) is persisted on the asset and Submission.Create requires it to equal the submission's pinned version + the answering field |
