...
**Judge access (competition demo):** the public `/judge` page posts `{ role }` to `POST /api/auth/judge-access`, which registers when `ROLE_SWITCHER_ENABLED=1` — otherwise the path 404s. **No access code.** Attempts are rate-limited (10 / 5 min / IP — best-effort per Worker isolate). Success mints a 4-hour demo-persona session on the server-fixed demo event (`evt_dogfood`). Demo sessions can create API keys, but expiry is clamped to at most 4 hours, and they may revoke only demo-created keys — seeded/owner keys stay protected. This is an open shared demo: anyone can mint the seeded admin/evaluator/speaker personas.
