...
| Path | When | How |
|------|------|-----|
| **A. Judge access** (`/judge`, judges) | Shared demo with `JUDGE_ACCESS_CODE` + `ROLE_SWITCHER_ENABLED=1` | Public `/judge` page exchanges the access code (`POST /api/auth/judge-access`) for a **4-hour** demo-persona session (`admin` / `evaluator` / `speaker`) on `evt_dogfood`. Route 404s when disabled; rate-limited 10 attempts / 5 min / IP (best-effort per Worker isolate, in-memory); demo sessions mint API keys clamped to a 4-hour expiry and may revoke only demo-created keys; role switcher badge shows “Shared demo”. |
